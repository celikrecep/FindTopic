...
#     paths = 'TTC-3600_Orj/ekonomi/c '+"("+(i+1).__str__()+")"+".txt"  # note C:
#     file = open(paths,"r")
#     text = file.read().strip().__str__()
#     text = text.translate( { ord(c):None for c in '\n\t\r\ufeff' } )
#     dataEkonomi["Description"] = text
#     dataEkonomi["Id"] = i
#     dataEkonomi["Category"] = "Ekonomi"
#     Datas.append(dataEkonomi.copy())
#     file.close()
#

#df = pd.DataFrame(Datas)
#df.to_csv("./duzenli.csv", sep = ',', index = False)

csvFile = pd.read_csv('duzenli.csv')
